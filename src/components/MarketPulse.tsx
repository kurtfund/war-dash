...
export default function MarketPulse() {
    ...
    return (
        <div className="absolute inset-0 overflow-y-auto custom-terminal-scroll p-1.5 space-y-1 pb-10">
            {pulseData.length === 0 && (
            ...
            {pulseData.map((item, idx) => (
                <div
                    key={item.symbol}
                    className={`p-2 rounded border backdrop-blur-md shadow-sm flex justify-between items-center gap-2 transition-colors duration-500 ${item.up ? 'border-yellow-500/10 bg-yellow-900/5' : 'border-red-500/10 bg-red-900/5'
                        }`}
                >
