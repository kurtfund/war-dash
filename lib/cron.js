...
function runCronJobs(io) {
    ...
    async function pollMarketData() {
        ...
        try {
            ...
            if (results.length > 0) {
                ...
                console.log(`✅ Market Pulse: Fetched ${results.length} tickers successfully (Tier: ${results.length === TICKER_SYMBOLS.length ? 'Full' : 'Partial'}).`);
                emitResults(results, io);
            } else {
                console.error('❌ Market Pulse: All fetch methods failed. Using local cache.');
                emitResults(lastKnownResults, io);
            }
        } catch (e) {
    ...
    function emitResults(validResults, io) {
        ...
        console.log(`📡 WebSocket: Emitting market_update with ${formatted.length} items. Prices: ${formatted.map(f => `${f.symbol}: ${f.price}`).join(', ')}`);
