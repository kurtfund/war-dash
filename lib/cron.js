...
function runCronJobs(io) {
    ...
    setInterval(async () => {
        try {
            // Use native fetch to the direct v8 chart API to bypass Yahoo's strict scraping block on the Render Cloud IPs
            const results = await Promise.all(TICKER_SYMBOLS.map(async t => {
                try {
                    const res = await fetch(`https://query1.finance.yahoo.com/v8/finance/chart/${t.symbol}?interval=1d`, {
